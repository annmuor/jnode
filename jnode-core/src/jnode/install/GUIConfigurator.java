...
public class GUIConfigurator {
	...
	private void initialize() {
		...
		for (String key : config) {
			...
			Component comp;
			Component comment;
			if (key.equalsIgnoreCase("binkp.server")
					|| key.equalsIgnoreCase("binkp.client")
					|| key.equalsIgnoreCase("fileecho.enable")
					|| key.equalsIgnoreCase("stat.enable")
					|| key.equalsIgnoreCase("jscript.enable")) {
				comp = new Checkbox();
			} else {
				comp = new TextField();
			}
