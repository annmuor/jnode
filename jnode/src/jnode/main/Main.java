...
public class Main {
	...
	public static class SystemInfo {
		...
		private String NDL;
		private final String version = "jNode/0.3.4";
	...
	public static enum Settings {
		JDBC_URL("jdbc.url"), JDBC_USER("jdbc.user"), JDBC_PASS("jdbc.pass"), POLL_DELAY("poll.delay"), POLL_PERIOD(
				"poll.period"), INFO_SYSOP("info.sysop"), INFO_LOCATION("info.location"), INFO_STATIONNAME(
				"info.stationname"), INFO_NDL("info.ndl"), INFO_ADDRESS("info.address"), BINKD_BIND("binkd.listen"), BINKD_PORT(
				"binkd.port"), BINKD_INBOUND("binkd.inbound"), BINKD_CLIENT("binkd.client"), BINKD_SERVER(
				"binkd.server"), LOG_LEVEL("log.level");
	...
	private Main(String configFile) {
		try {
			...
			String line;
			Pattern config = Pattern.compile("^(\\S+)\\s*=?\\s*([^\r\n]+)[\r\n]*$");
			...
			if (info.sysop == null || info.location == null || info.stationName == null || info.NDL == null
					|| addressline == null) {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			try {
				delay = Integer.valueOf(settings.get(Settings.POLL_DELAY.cfgline));
				period = Integer.valueOf(settings.get(Settings.POLL_PERIOD.cfgline));
			} catch (RuntimeException e) {
				logger.warn("Не указаны poll.delay/poll.period, используем 0/600");
			}
			try {
				port = Integer.valueOf(settings.get(Settings.BINKD_PORT.cfgline));
			} catch (RuntimeException e) {
				logger.warn("Не указан binkd.port, используем 24554");
			}
			try {
				loglevel = Integer.valueOf(settings.get(Settings.LOG_LEVEL.cfgline));
			} catch (RuntimeException e) {
				logger.warn("Не указан log.level, используем " + loglevel);
			}
			Logger.Loglevel = loglevel;

			logger.info("jNode Project запущен");
			if (settings.get(Settings.BINKD_SERVER.cfgline) != null) {
				Thread server = new Server(settings.get(Settings.BINKD_BIND.cfgline), port);
				server.start();
				server = null;
			}
			if (settings.get(Settings.BINKD_SERVER.cfgline) != null) {
			...
			logger.info("jNode Project завершен");
		}
